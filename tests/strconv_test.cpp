...
#ifndef SYMXX_STRCONV_TEST_CPP
#define SYMXX_STRCONV_TEST_CPP
...
namespace symxx::test
{
  SYMXX_TEST(strconv)
  {
    ...
    SYMXX_EXPECT_EQ((ExprParser<int>("-1-x").parse().to_string()), "-1-x");
    SYMXX_EXPECT_EQ((ExprParser<int>("-x-1").parse().to_string()), "-1*x-1");
    SYMXX_EXPECT_EQ((ExprParser<int>("-x-x").parse().to_string()), "-1*x-x");
    SYMXX_EXPECT_EQ((ExprParser<int>("-2x").parse().to_string()), "-2*x");
    ...
    SYMXX_EXPECT_EQ((nth_root<int>(3, 6).to_string()), "_3_/6");
    SYMXX_EXPECT_EQ((nth_root<int>(9, Rational<int>{1, 3}).to_string()), "1/3_9_/6561");
